...
export const SUSPICIOUS_TLDS = ['.buzz', '.click', '.country', '.fit', '.gq', '.kim', '.loan', '.mom', '.party', '.quest', '.rest', '.ru', '.tk', '.top', '.work', '.xyz'];
export const SENSITIVE_WORDS = /(login|verify|secure|account|password|wallet|bank|invoice|payment|free|gift|prize|airdrop|crypto|support|update|download)/i;
export const BRAND_IMPERSONATION_WORDS = /(apple|facebook|google|instagram|microsoft|netflix|paypal|whatsapp|windows|youtube)/i;
export const URL_SHORTENER_HOSTS = new Set([
  'bit.ly', 'cutt.ly', 'is.gd', 'ow.ly', 'rebrand.ly', 's.id', 'shorturl.at', 't.co', 'tiny.cc', 'tinyurl.com', 'trib.al'
]);
...
export function getUrlRisk(inputUrl) {
  ...
  if (score >= 60) {
    return { level: 'dangerous', score, reasons };
  }

  if (score >= 15) {
    return { level: 'suspicious', score, reasons };
  }
...
export function isDangerousDownload(downloadItem) {
  ...
  if (riskyExtension) {
    return {
      dangerous: true,
      reasons: uniqueReasons([`The download uses a high-risk file type (${riskyExtension}).`, ...risk.reasons])
    };
  }

  if (suspiciousArchive) {
    return {
      dangerous: true,
      reasons: uniqueReasons(['The archive name matches common malware lure words.', ...risk.reasons])
    };
  }
...
function isIpAddress(hostname) {
  return /^\d{1,3}(\.\d{1,3}){3}$/.test(hostname) || /^\[[a-f0-9:]+]$/i.test(hostname);
}
