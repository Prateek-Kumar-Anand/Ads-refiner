...
const { getUrlRisk, isDangerousDownload, isLikelyAdUrl } = globalThis.adsRefinerSafety;
...
async function updateAdRuleset() {
  ...
  const shouldEnable = runtimeSettings.enabled && runtimeSettings.blockAds;
  const update = shouldEnable
    ? { enableRulesetIds: ['ads_refiner_rules'], disableRulesetIds: [] }
    : { enableRulesetIds: [], disableRulesetIds: ['ads_refiner_rules'] };
